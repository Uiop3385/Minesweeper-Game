...
func _ready() -> void:
	...
	add_child(tabs_holder, true)
	for child in children:
		if(child is ScrollBar): continue
		remove_child(child)
		tabs_holder.add_child(child)
		tabs_holder.fit_child_in_rect(child, Rect2(Vector2(0, 0), Vector2(1080, 1920)))
...
func _manage_input(event: InputEvent) -> void:
	...
	elif event is InputEventScreenDrag and not ArrayManager.camera_locked:
		$MarginContainer/Minesweeper/SubViewportContainer/SubViewport.set_input_as_handled()
	...
	elif event is InputEventScreenTouch and not ArrayManager.camera_locked:
		$MarginContainer/Minesweeper/SubViewportContainer/SubViewport.set_input_as_handled()
		if event.is_pressed():
			scrolling = true
